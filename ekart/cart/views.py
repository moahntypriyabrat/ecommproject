...
def cart(request, total=0, quantity=0, cart_items=None):
    try:
        ...
        pass  # just ignore
    context = {
        'total': total, 'quantity': quantity, 'cart_items': cart_items, 'tax': tax, 'grand_total': grand_total
    }
    return render(request, 'ekartapp/cart.html', context)
...
def add_cart(request, product_id):
    ...
    product = Product.objects.get(id=product_id)  # to get the product in cart
...
def checkout(request, total=0, quantity=0, cart_items=None):
    ...
    return render(request, 'ekartapp/checkout.html', context)
